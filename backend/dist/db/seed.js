...
async function seed() {
    ...
    try {
        ...
        await database_1.default.query(`INSERT INTO staff_users (email, password_hash, name, role)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email) DO NOTHING`, ['admin@sarnies.com', adminPassword, 'Admin User', 'admin']);
        console.log('✅ Admin user created (admin@sarnies.com / admin123)');
        // Create staff user
        const staffPassword = await bcrypt_1.default.hash('staff123', 10);
        await database_1.default.query(`INSERT INTO staff_users (email, password_hash, name, role)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email) DO NOTHING`, ['staff@sarnies.com', staffPassword, 'Staff User', 'staff']);
        ...
        for (const voucher of vouchers) {
            await database_1.default.query(`INSERT INTO vouchers (
        ...
        for (const announcement of announcements) {
            await database_1.default.query(`INSERT INTO announcements (
        ...
        for (const customer of testCustomers) {
            const result = await database_1.default.query(`INSERT INTO users (name, surname, phone, birthday, company, gender, points_balance, static_qr_code)
         ...
         RETURNING id`, [
            ...
            const staticQR = await (0, qrCode_1.generateStaticQR)(userId);
            await database_1.default.query(`UPDATE users SET static_qr_code = $1, static_qr_image = $2, static_qr_created_at = $3 WHERE id = $4`, [staticQR.token, staticQR.dataUrl, staticQR.createdAt, userId]);
        }
