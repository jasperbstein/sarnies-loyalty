...
exports.generateRecurringVouchers = generateRecurringVouchers;
...
function isUserEligible(user, voucher) {
    ...
    switch (recurrence_type) {
        ...
        case 'weekly':
            ...
            if (!user.last_weekly_voucher_generated_at)
                return true;
        ...
        case 'monthly':
            ...
            if (!user.last_monthly_voucher_generated_at)
                return true;
 ...
 */
async function generateVoucherInstance(user, voucher) {
    try {
        // Calculate expiry
        let expiresAt;
        ...
        const qrToken = (0, jwt_1.generateQRToken)(qrData, '24h');
        // Create voucher instance
        await (0, database_1.query)(`INSERT INTO voucher_instances
       (uuid, user_id, voucher_id, qr_code_data, status, redeemed_at, expires_at, is_reusable)
       VALUES ($1, $2, $3, $4, $5, NOW(), $6, $7)`, [uuid, user.id, voucher.id, qrToken, 'active', expiresAt, false]);
 ...
 */
async function updateRecurrenceTimestamp(userId, recurrenceType) {
    const timestampColumn = recurrenceType === 'daily' ? 'last_daily_voucher_generated_at' :
        recurrenceType === 'weekly' ? 'last_weekly_voucher_generated_at' :
            recurrenceType === 'monthly' ? 'last_monthly_voucher_generated_at' :
                null;
    if (timestampColumn) {
        await (0, database_1.query)(`UPDATE users SET ${timestampColumn} = NOW() WHERE id = $1`, [userId]);
    }
}
/**
 * Main job function
 */
async function generateRecurringVouchers() {
    const startTime = Date.now();
    console.log('🔄 Starting recurring vouchers generation job...');
    try {
        // 1. Get all auto-generate vouchers
        const vouchersResult = await (0, database_1.query)(`SELECT * FROM vouchers
       ...
       ORDER BY id ASC`);
        ...
        for (const voucher of vouchers) {
            ...
            const usersResult = await (0, database_1.query)(`SELECT id, user_type, birthday, registration_date,
                last_daily_voucher_generated_at, last_weekly_voucher_generated_at,
                last_monthly_voucher_generated_at
         FROM users
         WHERE user_type = ANY($1)
         ORDER BY id ASC`, [voucher.target_user_types || ['customer', 'employee']]);
            ...
            for (const user of users) {
                if (isUserEligible(user, voucher)) {
                    ...
                    const existingResult = await (0, database_1.query)(`SELECT COUNT(*) as count FROM voucher_instances
             WHERE voucher_id = $1 AND user_id = $2
             AND status = 'active'
             AND DATE(redeemed_at) = CURRENT_DATE`, [voucher.id, user.id]);
                    const hasActiveInstance = parseInt(existingResult.rows[0].count) > 0;
                    if (!hasActiveInstance) {
                        // Check daily limit (if applicable)
                        if (voucher.max_redemptions_per_user_per_day) {
                            const todayCount = await (0, database_1.query)(`SELECT COUNT(*) as count FROM voucher_instances
                 WHERE voucher_id = $1 AND user_id = $2
                 AND DATE(redeemed_at) = CURRENT_DATE`, [voucher.id, user.id]);
...
//# sourceMappingURL=generateRecurringVouchers.js.map