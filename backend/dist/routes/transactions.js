...
router.post('/earn', auth_1.authenticate, auth_1.requireStaff, async (req, res) => {
    try {
        ...
        const userCheckResult = await (0, database_1.query)('SELECT user_type FROM users WHERE id = $1', [user_id]);
        ...
        const points = (0, points_1.calculatePoints)(amount);
        if (points === 0) {
        ...
        const result = await (0, database_1.transaction)(async (client) => {
         ...
         WHERE id = $3`, [points, amount, user_id]);
            // Create transaction record
            const txResult = await client.query(`INSERT INTO transactions (user_id, type, points_delta, amount_value, outlet, staff_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [user_id, 'earn', points, amount, outlet, req.user?.id]);
            // Get updated user
...
router.post('/redeem', auth_1.authenticate, auth_1.requireStaff, async (req, res) => {
    try {
        ...
        const result = await (0, database_1.transaction)(async (client) => {
            ...
            // Get voucher
            const voucherResult = await client.query('SELECT * FROM vouchers WHERE id = $1', [voucher_id]);
            ...
            if (!(0, points_1.validateRedemption)(user.points_balance, voucher.points_required)) {
                throw new Error('Insufficient points');
            }
            // Deduct points
            await client.query('UPDATE users SET points_balance = points_balance - $1 WHERE id = $2', [voucher.points_required, user_id]);
            // Create transaction record
            const txResult = await client.query(`INSERT INTO transactions (user_id, type, points_delta, amount_value, voucher_id, outlet, staff_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING *`, [user_id, 'redeem', -voucher.points_required, voucher.value_amount, voucher_id, outlet, req.user?.id]);
            // Get updated user
            const updatedUserResult = await client.query('SELECT * FROM users WHERE id = $1', [user_id]);
            return {
                transaction: txResult.rows[0],
