 ...
 */
router.get('/budget', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.post('/admin/:userId/budget', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        const { userId } = req.params;
        const { annualBudgetThb, spentThb = 0, expiresAt } = req.body;
        ...
        if (userCheck.rows[0].user_type !== 'media') {
            await client.query('UPDATE users SET user_type = $1 WHERE id = $2', ['media', userId]);
        }
      ...
      RETURNING
        ...
        media_spent_this_year_thb,
        media_budget_allocated_at,
        media_budget_expires_at`, [annualBudgetThb, spentThb, expiryDate, userId]);
        // Log transaction
 ...
 */
router.patch('/admin/:userId/budget/adjust', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const client = await database_1.default.connect();
    try {
        const { userId } = req.params;
      ...
      RETURNING
        media_annual_budget_thb,
        media_spent_this_year_thb`, [amountThb, userId]);
        if (result.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: 'User not found' });
        }
 ...
 */
router.get('/admin/budgets/expiring', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
      ...
      ORDER BY media_budget_expires_at ASC`, [days]);
        res.json({
 ...
 */
router.get('/admin/usage-report', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const result = await database_1.default.query(`SELECT
        u.id,
        u.name,
