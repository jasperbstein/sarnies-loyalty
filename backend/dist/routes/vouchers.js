...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        v.*,
        COALESCE(
          (SELECT COUNT(*) FROM voucher_instances vi
           WHERE vi.voucher_id = v.id
           AND vi.user_id = $2
           AND DATE(vi.redeemed_at) = CURRENT_DATE),
          0
        ) as redeemed_today,
        COALESCE(
          (SELECT COUNT(*) FROM voucher_instances vi
           WHERE vi.voucher_id = v.id
           AND vi.user_id = $2
           AND vi.status IN ('active', 'used')),
          0
        ) as total_redemptions
       FROM vouchers v
       WHERE v.is_active = true
       AND (v.expiry_date IS NULL OR v.expiry_date > NOW())
       AND ($1 = ANY(v.target_user_types) OR v.target_user_types IS NULL)
       AND (
         -- Recurring vouchers: check eligibility using helper function
         v.recurrence_type IS NULL
         OR v.recurrence_type = 'one_time'
         OR check_recurring_voucher_eligibility($2, v.id)
       )
       ORDER BY v.is_featured DESC, v.points_required ASC`, [userType, userId]);
...
// Get all vouchers including inactive with usage statistics (admin only)
...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)('SELECT * FROM vouchers WHERE id = $1', [id]);
...
// Check if user has redeemed this voucher today
router.get('/:id/today-redemption', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const result = await (0, database_1.query)(`SELECT COUNT(*) as count FROM voucher_instances
       WHERE voucher_id = $1 AND user_id = $2
       AND DATE(redeemed_at) = CURRENT_DATE`, [id, userId]);
        const redeemedToday = parseInt(result.rows[0].count) > 0;
        res.json({ redeemed_today: redeemedToday });
    }
    catch (error) {
        console.error('Check today redemption error:', error);
        res.status(500).json({ error: 'Failed to check redemption status' });
    }
});
// Get employee vouchers with today's availability (for staff scanner)
router.get('/employee/:userId/available', auth_1.authenticate, async (req, res) => {
    try {
        const { userId } = req.params;
        // Get all vouchers available to employees with eligibility check
        const vouchersResult = await (0, database_1.query)(`SELECT v.* FROM vouchers v
       WHERE v.is_active = true
       AND (v.expiry_date IS NULL OR v.expiry_date > NOW())
       AND 'employee' = ANY(v.target_user_types)
       AND (
         -- Check voucher eligibility using the same logic as main vouchers endpoint
         v.recurrence_type IS NULL
         OR v.recurrence_type = 'one_time'
         OR check_recurring_voucher_eligibility($1, v.id)
       )
       ORDER BY v.is_featured DESC, v.points_required ASC`, [userId]);
        // For each voucher, get today's redemption count
        const vouchersWithAvailability = await Promise.all(vouchersResult.rows.map(async (voucher) => {
            const redemptionsResult = await (0, database_1.query)(`SELECT COUNT(*) as count FROM voucher_instances
           WHERE voucher_id = $1 AND user_id = $2
           AND DATE(redeemed_at) = CURRENT_DATE`, [voucher.id, userId]);
            const todayRedemptions = parseInt(redemptionsResult.rows[0].count);
            const maxPerDay = voucher.max_redemptions_per_user_per_day || 999;
            const availableToday = Math.max(0, maxPerDay - todayRedemptions);
            return {
                ...voucher,
        ...
        res.json({ vouchers: vouchersWithAvailability });
    }
    catch (error) {
        console.error('Get employee vouchers error:', error);
        res.status(500).json({ error: 'Failed to fetch employee vouchers' });
    }
});
// Create voucher (admin only)
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        const { title, description, category, points_required, voucher_type, benefit_type, benefit_value, cash_value, is_active, is_featured, expiry_type, expiry_date, expiry_days, target_user_types, max_redemptions_per_user, max_redemptions_total, valid_from, valid_until, image_url, redemption_window, requires_minimum_purchase, valid_days_of_week, valid_outlets, auto_expire_hours } = req.body;
        // Sanitize title to prevent XSS
        const sanitizedTitle = (0, sanitize_1.sanitizeHtml)(title) || '';
        ...
        const duplicateCheck = await (0, database_1.query)('SELECT id FROM vouchers WHERE LOWER(title) = LOWER($1)', [title]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO vouchers (
       ...
       RETURNING *`, [
        ...
        // Audit log
        await (0, auditLog_1.logVoucherAction)(req, 'create', newVoucher.id, newVoucher.title, {
            after: newVoucher
        });
...
router.patch('/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const oldVoucherResult = await (0, database_1.query)('SELECT * FROM vouchers WHERE id = $1', [id]);
        ...
        if (title !== undefined) {
            ...
            const duplicateCheck = await (0, database_1.query)('SELECT id FROM vouchers WHERE LOWER(title) = LOWER($1) AND id != $2', [title, id]);
            if (duplicateCheck.rows.length > 0) {
                return res.status(400).json({ error: 'A voucher with this title already exists' });
            }
        }
        ...
        values.push(id);
        const result = await (0, database_1.query)(`UPDATE vouchers SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${paramCount} RETURNING *`, values);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Voucher not found' });
        }
        const updatedVoucher = result.rows[0];
        // Audit log
        await (0, auditLog_1.logVoucherAction)(req, 'update', updatedVoucher.id, updatedVoucher.title, {
            before: oldVoucher,
...
router.post('/:id/redeem', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Get voucher details
        const voucherResult = await (0, database_1.query)('SELECT * FROM vouchers WHERE id = $1', [id]);
        ...
        if (voucher.max_redemptions_per_user_per_day) {
            const todayRedemptionsResult = await (0, database_1.query)(`SELECT COUNT(*) as redemption_count FROM voucher_instances
         WHERE voucher_id = $1 AND user_id = $2
         AND status IN ('active', 'used')
         AND DATE(redeemed_at) = CURRENT_DATE`, [id, user_id]);
        ...
        if (voucher.max_redemptions_per_user) {
            const userRedemptionsResult = await (0, database_1.query)(`SELECT COUNT(*) as redemption_count FROM voucher_instances
         WHERE voucher_id = $1 AND user_id = $2 AND status IN ('active', 'used')`, [id, user_id]);
        ...
        if (voucher.max_redemptions_total) {
            const totalRedemptionsResult = await (0, database_1.query)(`SELECT COUNT(*) as redemption_count FROM voucher_instances
         WHERE voucher_id = $1 AND status IN ('active', 'used')`, [id]);
        ...
        if (voucher.is_free) {
            ...
            if (use_group_credits) {
                ...
                const outletCreditResult = await (0, database_1.query)('SELECT credits_balance FROM investor_outlet_credits WHERE user_id = $1 AND outlet_id = $2', [user_id, outlet_id]);
        ...
        const qrToken = (0, jwt_1.generateQRToken)(qrData, '10m');
        ...
        const instanceResult = await (0, database_1.query)(`INSERT INTO voucher_instances
       (uuid, user_id, voucher_id, qr_code_data, status, redeemed_at, expires_at, is_reusable)
       VALUES ($1, $2, $3, $4, $5, NOW(), $6, $7)
       RETURNING *`, [uuid, user_id, id, qrToken, 'active', expiresAt, voucher.is_reusable || false]);
        // Update recurrence timestamps for user (if recurring voucher)
        if (voucher.recurrence_type) {
            const timestampColumn = voucher.recurrence_type === 'daily' ? 'last_daily_voucher_generated_at' :
                voucher.recurrence_type === 'weekly' ? 'last_weekly_voucher_generated_at' :
                    voucher.recurrence_type === 'monthly' ? 'last_monthly_voucher_generated_at' :
                        null;
            if (timestampColumn) {
                await (0, database_1.query)(`UPDATE users SET ${timestampColumn} = NOW() WHERE id = $1`, [user_id]);
            }
        }
        ...
        if (currency === 'free' || cost === 0) {
            ...
            if (use_group_credits) {
                ...
                await (0, database_1.query)('UPDATE users SET investor_group_credits_balance = $1 WHERE id = $2', [newBalance, user_id]);
                // Log credit transaction
                await (0, database_1.query)(`INSERT INTO credit_transactions
           (user_id, credit_type, transaction_type, credits_change, balance_after, voucher_id, outlet_id)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [user_id, 'investor_group', 'redemption', -cost, newBalance, id, null]);
            }
            else {
                // Deduct from outlet credits
                const outletCreditResult = await (0, database_1.query)(`UPDATE investor_outlet_credits
           SET credits_balance = credits_balance - $1, updated_at = NOW()
           WHERE user_id = $2 AND outlet_id = $3
           RETURNING credits_balance`, [cost, user_id, outlet_id]);
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [user_id, 'investor_outlet', outlet_id, 'redemption', -cost, newBalance, id]);
            }
        }
        else if (currency === 'media_budget_thb') {
            // Deduct from media budget
            const newSpent = parseFloat(user.media_spent_this_year_thb) + cost;
            await (0, database_1.query)('UPDATE users SET media_spent_this_year_thb = $1 WHERE id = $2', [newSpent, user_id]);
            // Log credit transaction
            await (0, database_1.query)(`INSERT INTO credit_transactions
         (user_id, credit_type, transaction_type, amount_thb, voucher_id, outlet_id)
         VALUES ($1, $2, $3, $4, $5, $6)`, [user_id, 'media', 'redemption', cost, id, outlet_id]);
        }
        else {
            // Deduct regular points
            const newBalance = user.points_balance - cost;
            await (0, database_1.query)('UPDATE users SET points_balance = $1 WHERE id = $2', [newBalance, user_id]);
            // Create transaction record
            await (0, database_1.query)(`INSERT INTO transactions
         (user_id, type, points_delta, amount_value, voucher_id)
         VALUES ($1, $2, $3, $4, $5)`, [user_id, 'redeem', -cost, voucher.cash_value, id]);
        }
        res.json({
            voucher_instance: {
...
router.get('/instances/:user_id', auth_1.authenticate, async (req, res) => {
    try {
...
router.delete('/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)('DELETE FROM vouchers WHERE id = $1 RETURNING id', [id]);
        // Audit log
        await (0, auditLog_1.logVoucherAction)(req, 'delete', deletedVoucher.id, deletedVoucher.title, {
