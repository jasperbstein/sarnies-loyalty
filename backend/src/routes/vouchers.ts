  ...
  try {
    ...
    const result = await query(
      `SELECT
       ...
       AND ($1 = ANY(v.target_user_types) OR v.target_user_types IS NULL OR v.target_user_types = '{}')
       ORDER BY v.is_featured DESC, v.points_required ASC`,
      [userType, userId]
    );
...
router.get('/:id/today-redemption', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.get('/employee/:userId/available', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { userId } = req.params;

    // Single optimized query with LEFT JOIN to get redemption counts
    const result = await query(
      `SELECT
       ...
       LEFT JOIN (
         ...
         WHERE user_id = $1 AND DATE(redeemed_at) = CURRENT_DATE
         GROUP BY voucher_id
       ) today_counts ON v.id = today_counts.voucher_id
       WHERE v.is_active = true
       AND (v.expiry_date IS NULL OR v.expiry_date > NOW())
...
// Get customer's available/affordable vouchers (for staff scanner EligibleRewardsCard)
// Returns vouchers the customer can afford based on their points balance
router.get('/customer/:customerId/available', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { customerId } = req.params;

    // Get the customer's current points balance
    const userResult = await query(
      'SELECT id, points_balance, user_type FROM users WHERE id = $1',
      [customerId]
    );
    ...
    const userType = customer.user_type || 'customer';

    // Get vouchers the customer is eligible for and can afford
    const result = await query(
      `SELECT
        v.id,
        v.title,
        ...
        CASE
          WHEN v.points_required <= $2 THEN true
          ELSE false
        END as can_afford,
        CASE
          WHEN v.points_required <= $2 THEN 'Has enough points'
          ELSE 'Needs ' || (v.points_required - $2) || ' more points'
       ...
       LEFT JOIN (
         SELECT
           ...
           COUNT(*) FILTER (WHERE DATE(redeemed_at) = CURRENT_DATE) as today_count
         FROM voucher_instances
         WHERE user_id = $1 AND status IN ('active', 'used')
         GROUP BY voucher_id
       ) user_redemptions ON v.id = user_redemptions.voucher_id
       WHERE v.is_active = true
       AND (v.expiry_date IS NULL OR v.expiry_date > NOW())
       AND ($3 = ANY(v.target_user_types) OR v.target_user_types IS NULL OR v.target_user_types = '{}')
       AND (
       ...
       ORDER BY
         ...
         v.is_featured DESC,
         v.points_required ASC`,
      [customerId, pointsBalance, userType]
    );
    ...
    console.error('Get customer available vouchers error:', error);
    res.status(500).json({ error: 'Failed to fetch customer available vouchers' });
  }
});
