...
router.post('/subscribe', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await subscribeToPush(userId, subscription, userAgent);
...
router.get('/history', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, notification_type, title, body, data, category, status, sent_at, created_at
...
router.post('/admin/send', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const userId of user_ids) {
      await queueNotification(userId, 'admin_message', title, body, category, data);
      const result = await sendPushToUser(userId, {
        title,
        body,
 ...
 */
router.post('/admin/broadcast', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const userId of userIds) {
      await queueNotification(userId, 'broadcast', title, body, category, data);
    }
 ...
 */
router.post('/admin/process-queue', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    await processNotificationQueue();
