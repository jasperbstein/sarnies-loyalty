...
import { logAnnouncementAction } from '../utils/auditLog';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const userType = (req.query.user_type as string) || 'customer';
    const now = new Date();
...
router.post('/', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Audit log
    await logAnnouncementAction(req, 'create', newAnnouncement.id, newAnnouncement.title, {
...
router.patch('/:id', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const oldAnnouncementResult = await query('SELECT * FROM announcements WHERE id = $1', [id]);
    ...
    const result = await query(
      `UPDATE announcements SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${paramCount} RETURNING *`,
      values
    );
    ...
    // Audit log
    await logAnnouncementAction(req, 'update', updatedAnnouncement.id, updatedAnnouncement.title, {
...
router.delete('/:id', authenticate, requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const announcementResult = await query('SELECT * FROM announcements WHERE id = $1', [id]);
    ...
    // Audit log
    await logAnnouncementAction(req, 'delete', deletedAnnouncement.id, deletedAnnouncement.title, {
