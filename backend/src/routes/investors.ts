...
import { InvestorOutletCredit, CreditTransaction } from '../types';
...
router.get('/credits', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const outletCreditsResult = await pool.query(
      `SELECT
        ioc.*,
 ...
 */
router.get('/credits/outlet/:outletId', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { outletId } = req.params;

    const result = await pool.query(
      'SELECT * FROM get_investor_credits_for_outlet($1, $2)',
      [userId, outletId]
    );
 ...
 */
router.post('/admin/:userId/outlet-credits', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const creditResult = await client.query(
      `INSERT INTO investor_outlet_credits (
      ...
      DO UPDATE SET
        ...
        updated_at = NOW()
      RETURNING *`,
      [userId, outletId, balance, annualAllocation, expiryDate, autoRenew]
    );
 ...
 */
router.post('/admin/:userId/group-credits', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const result = await client.query(
      ...
      RETURNING
        ...
        investor_group_credits_allocated_at,
        investor_group_credits_expires_at`,
      [enabled, balance, annualAllocation, expiryDate, userId]
    );
 ...
 */
router.patch('/admin/:userId/credits/adjust', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  const client = await pool.connect();
    ...
    if (creditType === 'investor_outlet') {
      ...
      const result = await client.query(
        ...
        SET investor_group_credits_balance = investor_group_credits_balance + $1,
            updated_at = NOW()
        WHERE id = $2
        RETURNING investor_group_credits_balance`,
        [amount, userId]
      );
 ...
 */
router.get('/admin/credits/expiring', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const groupResult = await pool.query(
      ...
      ORDER BY investor_group_credits_expires_at ASC`,
      [days]
    );
