...
export async function renewInvestorCredits() {
  ...
  try {
    ...
    const outletResult = await client.query(
      `UPDATE investor_outlet_credits
       ...
       RETURNING user_id, outlet_id, credits_balance, auto_renew`,
      [nextYearEnd]
    );
    ...
    const groupResult = await client.query(
      `UPDATE users
       ...
       RETURNING id, investor_group_credits_balance`,
      [nextYearEnd]
    );
...
export async function resetMediaBudgets() {
  const client = await pool.connect();
  ...
  try {
    ...
    const result = await client.query(
      `UPDATE users
       ...
       RETURNING id, media_annual_budget_thb`,
      [nextYearEnd]
    );
...
// Run both renewals
export async function runAnnualRenewal() {
